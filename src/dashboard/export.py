...
EXPORT_TABS = {"expenses", "income", "main", "year", "month", "planning", "input", "debts", "investments"}
_EXPORT_LOCK = Lock()
...
def build_dashboard_url(
    ...
    params = {"currency": currency, "tab": "main" if tab in {"expenses", "income"} else tab}
    if tab in {"expenses", "income"}:
        params["section"] = tab
    if year is not None:
        params["year"] = year
